      ...
      <div className="checkout-container flex flex-col items-center justify-center min-h-screen bg-gray-100 p-4">
        ...
        <form onSubmit={submitHandler} className="bg-white p-6 rounded-lg shadow-md max-w-lg w-full">
          <div className="text-center mb-6">
            <h1 className="text-3xl font-bold text-blue-600">SMart</h1>
          </div>
...
const Checkout: React.FC = () => {
  ...
  useEffect(() => {
    if (!stripePublishableKey) {
      setStripeError("Stripe publishable key is missing. Please set VITE_PUBLISHABLE_KEY in your .env file.");
      setStripeLoaded(false);
      return;
    }
    // Wait for stripePromise to resolve
    (async () => {
      try {
        const stripe = await stripePromise;
        if (!stripe) {
          setStripeError("Stripe.js failed to load. Please check your internet connection and disable ad blockers.");
        } else {
          setStripeLoaded(true);
        }
      } catch (err) {
        setStripeError("Stripe.js failed to load.");
      }
    })();
  }, []);
  ...
  useEffect(() => {
    const fetchPaymentIntent = async () => {
      try {
        // Convert total to paise before sending to backend
        const data = await createPaymentIntent({ amount: Math.round(total * 100) }).unwrap();
  ...
  if (stripeError) {
    return <div className="text-center text-red-500 mt-10">{stripeError}</div>;
  }
